...
# ========== УНИВЕРСАЛЬНЫЕ ФУНКЦИИ ДЛЯ ФАЙЛОВ ==========
async def send_file_to_user(user_id, file_id, file_type, caption=""):
    """Универсальная отправка файлов пользователю"""
    try:
        if file_type == "photo":
        ...
        print(f"Ошибка отправки файла пользователю: {e}")
        return False

async def send_file_to_admin(admin_id, file_id, file_type, caption=""):
    """Универсальная отправка файлов админу"""
    try:
        if file_type == "photo":
        ...
        print(f"Ошибка отправки файла админу: {e}")
        return False

async def show_file_preview(message, file_id, file_type, caption=""):
    """Показывает файл в превью (для истории)"""
    try:
        if file_type == "photo":
        ...
        await message.answer(f"❌ Не удалось показать файл: {e}")
...
def create_ticket(user_id, username, question, file_id=None, file_type=None, file_caption=None):
    global next_ticket_id
    ticket_id = str(next_ticket_id)
    next_ticket_id += 1
    
    message_data = {"role": "user", "text": question or "📎 Файл", "time": datetime.now().isoformat()}
    if file_id:
        message_data["file_id"] = file_id
        message_data["file_type"] = file_type
        message_data["file_caption"] = file_caption or question
...
• 📜 **История** — вся переписка с возможностью просмотра файлов
...
async def create_ticket_start(message: types.Message, state: FSMContext):
    ...
    await message.answer("📝 Напишите ваш вопрос или отправьте фото/файл:")
...
async def create_ticket_submit(message: types.Message, state: FSMContext):
    ...
    if message.text:
        ...
        file_type = "document"
        text = message.caption or f"📄 {message.document.file_name}"
    elif message.video:
        file_id = message.video.file_id
        file_type = "video"
        text = message.caption or "🎥 Видео"
    elif message.audio:
        file_id = message.audio.file_id
        file_type = "audio"
        text = message.caption or f"🎵 {message.audio.file_name}"
    elif message.voice:
    ...
    ticket_id = create_ticket(user_id, username, text, file_id, file_type, message.caption)
    
    # Уведомление админам с файлом, если есть
    free_admins = get_free_admins()
    ...
    for admin_id in free_admins:
        ...
        if file_id:
            msg_text += f"\n📎 **Есть вложение**"
            keyboard = InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="📋 Открыть обращение", callback_data=f"view_{ticket_id}")]
            ])
            await bot.send_message(admin_id, msg_text, reply_markup=keyboard)
            await send_file_to_admin(admin_id, file_id, file_type, f"Вложение к обращению #{ticket_id}")
        else:
            keyboard = InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="📋 Открыть список", callback_data="back_to_list")]
            ])
...
async def user_history(callback: types.CallbackQuery):
    ...
    # Отправляем историю сообщениями
    await callback.message.answer("📜 **История переписки:**")
    
    for idx, msg in enumerate(t["messages"]):
        role = "👤 Вы" if msg["role"] == "user" else f"🛠 {msg.get('admin_name', 'Оператор')}"
        time = datetime.fromisoformat(msg["time"]).strftime("%H:%M %d.%m")
        
        text_msg = f"{role} ({time}):\n{msg['text'][:300]}"
        ...
        if msg.get("file_id"):
            ...
            # Отправляем файл
            await show_file_preview(callback.message, msg["file_id"], file_type)
        else:
            await callback.message.answer(text_msg)
...
async def answer_ticket_send(message: types.Message, state: FSMContext):
    ...
    # Обработка разных типов контента от админа
    text = ""
    file_id = None
    file_type = None
    ...
    caption = f"📨 **Оператор {admin_name}:**\n\n{text}"
    
    if file_id:
        await send_file_to_user(t["user_id"], file_id, file_type, caption)
    else:
        await bot.send_message(
            t["user_id"],
...
async def close_ticket(callback: types.CallbackQuery):
    ...
    # Замер времени ответа для статистики
    if t["messages"]:
...
async def ticket_history(callback: types.CallbackQuery):
    ...
    await callback.message.answer(f"📜 **История обращения #{ticket_id}:**")
    
    for idx, msg in enumerate(t["messages"]):
        role = "👤 Пользователь" if msg["role"] == "user" else f"🛠 {msg.get('admin_name', 'Оператор')}"
        time = datetime.fromisoformat(msg["time"]).strftime("%H:%M %d.%m")
...
async def user_message_in_chat(message: types.Message, state: FSMContext):
    ...
    for tid, t in tickets.items():
        if t["user_id"] == user_id and t["status"] == "active":
            # Обработка разных типов контента от пользователя
            text = ""
            file_id = None
            file_type = None
            ...
            caption = f"📨 **Пользователь @{t['username']} (#{tid}):**\n\n{text}"
            
            if file_id:
                await send_file_to_admin(admin_id, file_id, file_type, caption)
            else:
                await bot.send_message(
                    admin_id,
